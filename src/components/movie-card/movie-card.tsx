...
const MovieCard: React.FC<MovieCardProps> = (props) => {
    ...
    const {title, poster_path, vote_average, release_date, overview, genres,} = props

    const trimmedGenres  = genres.length > 4 ? genres.slice(0, 4) : genres
    ...
    return(
        <li className='card'>
            ...
            <div className='info'>
                ...
                <ul className='genres'>
                    { trimmedGenres.map(genre => <li key={genre.id}>{ genre.name }</li>) }
                </ul>
            </div>
